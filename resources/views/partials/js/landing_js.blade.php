...
function showVerify() {
    ...
    if (forgot == 1) {
        ...
        issue_password.style.backgroundColor = '#E6F2E6';
        issue.innerHTML =
            "<strong>Find Username</strong><br>Use your phone number to find your username";
        ...
        if (userPhone.value != "") {
            ...
            req.done(function(resp) {
                ...
                if (resp.ok) {
                    ...
                    issue_password.style.borderRadius = '10px';
                    issue.style.color = '#ac585a';
                    issue.style.backgroundColor = '#F2DEDE';
                    console.log(userPhone.value.length);
                    if (userPhone.value.includes("+") && userPhone.value.length != 13 || !userPhone.value
                        .includes("+") && userPhone.value.length != 10) {
                        issue.innerHTML =
                            "<strong>Invalid phone number</strong><br>No Zeraki Analytics accounts have been linked to 0715223003";
                    } else {
                        issue.innerHTML =
                            "<strong>There was a Problem.</strong><br/>Please contact the school's D.O.S / H.O.D Academics and request them to add your correct phone number to this account!";
                    }
        ...
        if (userIdentity.value != "" && userPhone.value == "") {
            ...
            req.done(function(resp) {
                if (resp.msg == "fail") {
                    ...
                    issue.style.backgroundColor = '#F2DEDE';
                    issue.innerHTML =
                        "<strong>Invalid username</strong><br>We cannot find an account with that username. please contact the class teacher/ Dean of studies for assitance, or click Forgot Username below";
                    setTimeout(() => {
                ...
                if (resp.password) {
                    setTimeout(() => {
                        ...
                        input_phone.classList.add('active-state');
                        code.classList.add('active-state');
                        school_name.innerHTML = userIdentity.value.includes("@") ? "<span>" +
                            userIdentity.value + "</span>" : "<span>" + userIdentity.value +
                            "@bibirionihigh</span>"
        ...
        if (userPhone.value != "" && resetcode.value == '') {
            ...
            if (userPhone.value.includes("+") && userPhone.value.length != 13 || !userPhone.value.includes("+") &&
                userPhone.value.length != 10) {
                pass_logo.classList.add('active-state');
                issue_password.classList.remove('active-state');
                issue.classList.remove('active-state');
                issue.innerHTML = "<strong>There was a Problem.</strong><br/>Please specify a valid phone number!";
                return;
            } else if ("0" + identiphone.innerHTML.substring(4, identiphone.innerHTML.length).localeCompare(userPhone
                    .value) < 0 || "+254" + identiphone.innerHTML.substring(1, identiphone.innerHTML.length)
                .localeCompare(userPhone.value) < 0) {
                console.log(userPhone.value.localeCompare(identiphone.innerHTML));
                pass_logo.classList.add('active-state');
                issue_password.classList.remove('active-state');
                issue.classList.remove('active-state');
                issue.innerHTML = "<strong>There was a Problem</strong><br/>The number" + userPhone.value +
                    " is not linked to " + userIdentity.value +
                    ". <br/> Please contact the school's D.O.S / H.O.D Academics and request them to add your correct phone number to this account.";
                return;
            }
            input_phone.classList.add('active-state');
