...
class HomeController extends Controller
{
    ...
    public function sendCode(Request $req)
    {
        ...
        try {
            ...
            // return json_encode($result);
            return json_encode([
                "msg" => "success", 'to' => $code[0]->phone, 'code' => $message, 'from' => env('TALKING_USERNAME'),
                'result' => $result
            ]);
        } catch (Exception $e) {
            echo "Error: " . $e->getMessage();
    ...
    public function dashboard()
    {
        ...
        $d['teacher'] = $this->teacher->findTeacherbyUserID(Auth::user()->id);

        $d['teachernum'] = $this->teacher->getTeacherNum();
        $d['studentnum'] = $this->my_class->getStudentNum();
        $d['staffnum'] = $this->my_class->getStaffNum();
        $d['class_subjects'] = $d['teacher'] != null ? $this->my_class->getClassSubjects($d['teacher']->id) : [];
        $d['grade_subjects'] = $this->exam->getGradeRecord();
        $d['all_groups'] = $this->teacher->getAllGroup();
        $d['types'] = Qs::getUserType();
        foreach ($d['all_forms'] as $key => $val) {
            if ($key === 0) {
                $val->mark =  [
                    'MeanPoints' => 0,
                    'Meanpointsdev' => 0,
