...
class PrintOutsController extends Controller
{
    ...
    public function meritList()
    {
        ...
        $d['form'] = $this->exam->get_all_form();
        $d['subjects'] = $this->exam->get_all_form();
        return view('pages.support_team.printouts.meritList', $d);
    }
    ...
    public function west()
    {
        ...
        $d['form'] = $this->exam->get_all_form();
        $d['subjects'] = $this->exam->get_all_form();
        return view('pages.support_team.printouts.west', $d);
    }
    ...
    public function get_meta_list(Request $req)
    {
        ...
        foreach ($form_stream as $key => $val) {
            ...
            $exam_records = $this->exam->exam_records(155, $my_class_id);
            $totalmark = 0;
    ...
    public function get_meta_data_for_report_form(Request $req)
    {
        ...
        foreach ($students as $each_stu) {
                ...
                foreach ($subjects_student as $entity) {
                    if ($subject === $entity->af) {
                        ...
                        array_push($tabledata, [
                            ...
                            "mark" => ($mark * 100) . "%",
                            "dev" => number_format(($mark - $mark_pre) * 100, 2),
                            "grade" => $grade_name,
                            "rank" => $rank,
                            "comment" => $comment,
            ...
            $data = [
                ...
                "Total_got_marks" => $Total_got_marks,
                "deviation_marks" => $Total_got_marks - $Total_got_marks_pre,
                "totalpoint" => $Total_point,
                "dev_point" => $Total_point - $Total_point_pre,
                "over_order" => $over_order,
                "mean_marks" => $meanmark,
                "dev_mean_mark" => number_format($meanmark - $meanmark_pre, 2),
                "meangrade" => $meangrade,
