...
class MyClassRepo
{
    ...
    {
        return count(StaffRecord::orderBy('id', 'asc')->get());
    }

    public function getStudentNum()
    {
        return count(Student::orderBy('id', 'asc')->get());
    }
    ...
    {
        return MyClass::where('id', $stream_id)->first();
    }
    public function get_student_name($val)
    {
        return Student::where('id', $val)->with("user")->first();
    }
    public function get_stream_teacher_name($stream_id)
    {
        return MyClass::where("id", $stream_id)->with('teacher.user')->first();
    }

    public function getteacher($val, $stream_id)
    {
        return ClassSubject::where(["subject_id" => $val, "my_class_id" => $stream_id])->with("teacher.user")->first();
    }

    public function getformstream($form_id)
    {
        return MyClass::where("form_id", $form_id)->with("teacher.user")->get();
    }

    public function get_userlist_from_exam_record($stream_id, $exam_id)
    {
        return ExamRecord::with(["student.user", "student.my_class"])->where(['exam_id' => $exam_id, 'my_class_id' => $stream_id])->get();
    }

    public function get_subject_list($stream_id)
    {
        return ClassSubject::with("subject")->where("my_class_id", $stream_id)->get();
    }
    ...
    {
        return MyClass::where('id', $id)->first();
    }
    ...
    public function examform_table_update($id, $data)
    {
        return ExamForm::find($id)->update($data);
    }
    ...
    {
        return ClassSubject::with(['subject.subject_type', 'subject.exam_record', 'my_class.form.exam_form', 'teacher', 'students_taken_this_subject'])->where('teacher_id', $teacher_id)->get();
    }
    //not used yet
    ...
    public function getMyClassSubjects($classId)
    { // filter subject that is taken by this class
    ...
    public function findFormbyteahcer_id($id)
    {
        return Form::where('teacher_id', $id)->get();
    }

    public function get_streams($form_id)
    {
        return MyClass::where("form_id", $form_id)->get();
    }

    public function get_subject_names($stream_id)
    {
        return ClassSubject::where("my_class_id", $stream_id)->get();
    }
}